...
This script expects the raw data files to be in `data-raw/noa/`. These files, including the raw survey data, are in a zip compressed file on OSF: <https://osf.io/cjd6z/>. 
...
Response rate: All who consented, so rows in this table

```{r}
percent(nrow(ac) / 342825, accuracy = .01)
```
...
Many start times are the same (or later) than the end time
...
Session duration doesn't match when we calculate it by hand
...
These data are from September 2020 onwards, so shouldn't have early start dates

```{r}
table(gt$session_start < ymd("2020-09-01"))
table(gt$session_end < ymd("2020-09-01"))
```
...
Filter out sessions that aren't in the two weeks preceding each player's survey
