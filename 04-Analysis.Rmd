...
```{r}
ac <- read_rds(here("data/noa/ac.rds"))
...
dat <- bind_rows(
  ...
  mutate(Study = factor(Study, labels = c("PVZ", "AC!")))
```
...
x1 <- dat %>% 
  group_by(Study) %>%
  group_modify(~tidy(lm(active_play ~ Hours, data = .x), conf.int = TRUE))
...
x2 <- dat %>% 
  group_by(Study) %>%
  group_modify(~tidy(lm(spane_balance ~ Hours, data = .x), conf.int = TRUE))
...
x3 <- dat %>% 
  group_by(Study) %>% 
  group_modify(
    ~tidy(lm(spane_balance ~ active_play, data = .x), conf.int = TRUE)
    ) 
...
dat <- dat %>% 
  mutate(
    ...
    Hours = as.numeric(scale(Hours / 10, T, F))
    )
...
p1 <- tmp %>%   
  ggplot(aes(estimate, term, col = Model)) +
