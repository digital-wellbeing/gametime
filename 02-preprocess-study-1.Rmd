...
In total, there are nine telemetry files, each one recording a different aspect of play events in Plants vs. Zombies: Battle for Neighborville (the data were collected in two waves, so technically there are 18 telemetry files), and one file for the survey results.
...
Because there were two of each telemetry files (waves), we create a function to read both waves' data into a single table:
