...
survey %>% 
  select(UID, date) %>% 
  mutate(wave = ifelse(date <= ymd("2020-08-31"), "Wave 1", "Wave 2")) %>% 
  group_by(wave) %>% 
...
```{r}
table(pvz$straightliner_motivations|pvz$straightliner_spane)
pvz <- filter(pvz, !straightliner_motivations)
pvz <- filter(pvz, !straightliner_spane)
```
