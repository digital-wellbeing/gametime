...
```{r univariate-scales}
tmp <- dat %>%
  mutate(multiplier = ifelse(Game=="PvZ", -.1, .1)) %>%
...
tmp <- tmp %>% 
  mutate(
    name = factor(
      name, 
      levels = c(
        "extrinsic", "enjoyment", "spane_balance",
        "autonomy", "competence", "relatedness"
      ),
      labels = c(
        "Extrinsic motivation", "Intrinsic motivation", "Player well-being",
        "Player autonomy", "Player competence", "Player relatedness"
      )
    )
  )
tmp2 <- tmp %>% 
  group_by(Game, multiplier, name) %>% 
...
#   geom_vline(
#     data = tmp2, 
#     aes(xintercept = value, color = Game), 
#     alpha = .5, size = 2
#   )
```
