...
tmp2 <- tmp %>% 
  group_by(Study, multiplier, name) %>% 
...
#   geom_vline(
#     data = tmp2, 
#     aes(xintercept = value, color = Study), 
#     alpha = .5, size = 2
#   )
```
...
tmp <- dat %>% 
  group_by(Study) %>% 
  summarise(
    Hours_m = mean(Hours, na.rm = TRUE),
    active_play_m = mean(active_play, na.rm = TRUE),
    Hours_sd = sd(Hours, na.rm = TRUE),
    active_play_sd = sd(active_play, na.rm = TRUE)
  )
dat %>%  
  ggplot(aes(col = Study, fill = Study)) +
  coord_cartesian(ylim = c(-1, 1)) +
  ...
  geom_histogram(
    ...
    bins = 30, alpha = 0.5, col = "white"
  ) +
  geom_point(
    data = tmp, aes(x = Hours_m, y = -0.98),
    shape = 25, size = 2.25
  ) +
  geom_point(
    data = tmp, aes(x = active_play_m, y = -0.98),
    shape = 25, size = 2.25, alpha = 0.5
  ) +
  labs(y = "Normalized count") +
